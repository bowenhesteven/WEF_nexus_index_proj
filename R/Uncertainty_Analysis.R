...
indicator_transformation_option = c("1", "2")
indicator_normalization_option = c("3", "4", "5")
pca_selection_option = c("6", "7", "8") 
pca_rotation_option = c("9", "10", "11", "12", "13", "14")
weight_scheme_option = c("15", "16", "17")
...
sample_calc <- function(input, sample_number) {
  ...
  for (i in 1:sample_number) {
    ...
    wef <- wef_calc(input, indicator_transformation, indicator_normalization, pca_selection, pca_rotation,  weight_scheme)
    
    sample_results <- append(sample_results, list(wef))
...
samples_df <- UA_samples$sample_results[[1]]$wef_rank %>% dplyr::select('NAME')
for (i in 1:length(UA_samples$sample_results)) {
  samples_df <- samples_df %>%
    left_join(UA_samples$sample_results[[i]]$wef_rank %>%
...
# baseline
Baseline <- wef_baseline$wef_rank %>% 
