# Water (W)-Energy (E)-Food (F) Index Model Construction Functions
library(dplyr)
library(stats)
library(GPArotation)
library(sf)
...
equal_sum <- function(pca_cardinal_assigned){
  ...
  wef_score <- list(score = score, CT_tab = CT_tab)
}
...
rank_process <- function(wef_score){
  wef_rank <- cbind(wef_score$CT_tab, wef_score$score) %>%
    mutate (rank = rank(.$score, ties.method = 'random'))
}


# WEF score/rank calculation function
wef_calc <- function(input, 
  ...
  if(weight_scheme == 15){
    wef_score <- equal_sum(pca_cardinal_assigned)
  }else if(weight_scheme == 16){
    wef_score <- first_component(pca_cardinal_assigned)
  }else if(weight_scheme == 17)
    wef_score <- weighted_sum(pca_cardinal_assigned)
  
  wef_rank <- rank_process(wef_score)
  
  results = list(wef_score = wef_score, wef_rank = wef_rank)
