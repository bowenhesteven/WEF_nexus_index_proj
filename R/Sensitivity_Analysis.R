...
wef_sa_fun <- function(input, mat){
  ...
  for (i in 1:sample_size) {
    ...
    wef_rank <- wef_calc(input, indicator_transformation, indicator_normalization, pca_selection, pca_rotation,  weight_scheme) %$% 
      wef_rank %>%
      dplyr::select('NAME', 'score', 'rank') %>% 
      st_drop_geometry()
    
    score[i, ] <- as.vector(t(wef_rank %>% dplyr::select('score')))
    rank[i, ] <- as.vector(t(wef_rank %>% dplyr::select('rank')))
  }
...
y <- wef_sa_fun(input = variable, mat)
